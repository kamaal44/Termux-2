...
if test -d ${HOME}/makedeb/${project_folder}; then
	echo "[makedeb-${package_name}] project exists: ${HOME}/makedeb/${project_folder}"
	exit
else
	echo "[makedeb-${package_name}] creating project: ${HOME}/makedeb/${project_folder}"
...
echo "[makedeb-${package_name}] type \"done\" when you're done moving file"
while true; do
	...
	if test -f ${copy_file}; then
		...
		echo "[makedeb-${package_name}] file not exists: ${copy_file}"
	fi
done
...
echo "[makedeb-${package_name}] ready to enter editor mode"
...
echo "done"
echo "[makedeb] output: ${HOME}/makedeb/${project_folder}.deb"
echo "[makedeb] close the program."
}
